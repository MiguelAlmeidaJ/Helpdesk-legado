...
if(isset($_POST["id"])){
  ...
  $pdo = ConnectionN3();
  $show = $pdo->prepare("SELECT cads_tipo_servi.* FROM cads_tipo_servi WHERE cads_tipo_servi.id = '$id'");
  $show->execute();
  $row=$show->fetch(PDO::FETCH_ASSOC);
  $tipo_servi=$row["servico"];
  $class_contab_id=$row["class_contab"];
  $status=$row["status"];
?>          

              <div class="form-group row my-1">
                <label class="col-4 col-form-label text-right">Tipo de Servico:</label> 
                <div class="col-8">
                  <div class="input-group">
                    <div class="input-group-prepend">
                      <div class="input-group-text"><i class="fas fa-tag"></i></div>
                    </div> 
                    <input name="servico" type="text" class="form-control" required="required" value="<?php echo $tipo_servi; ?>">
