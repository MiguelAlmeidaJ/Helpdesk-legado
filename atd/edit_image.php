...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("SELECT img_atd FROM imagens WHERE id = :img_id");
    ...
    if (isset($_FILES['editImagemInput']) && $_FILES['editImagemInput']['error'] === UPLOAD_ERR_OK) {
        $img_atd = file_get_contents($_FILES['editImagemInput']['tmp_name']);
    } else {
        $img_atd = $imagem['img_atd'];
    }

    // Atualiza a imagem
    $stmt = $pdo->prepare("UPDATE imagens SET img_atd = :img_atd, user_id = :user_id, data_atualizacao = NOW() WHERE id = :img_id");
