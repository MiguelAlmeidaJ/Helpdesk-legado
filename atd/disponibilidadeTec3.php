...
");
    ...
    $stmtAtendimentosEspera = $pdo->prepare("
    SELECT usuarios.user_id, 
        usuarios.user_nome, 
        usuarios.user_funcao, 
        atendimentos.id AS tarefa_id,
                                    ...
                                    <div class="card-body atd-list">
                                        <div style="color:black;">
                                            ...
                                            foreach ($dadosTecnicos['esperaAgrupada'] as $nivel => $atendimentos) :
                                                ...
                                                // Adicionar borda inferior se não for o último grupo
                                                $borderStyle = ($currentNivel < $totalNiveis) ? 'border-bottom: 1px solid #ddd;' : '';

                                                echo '<div style="padding: 5px 0; ' . $borderStyle . '">';
                                                echo '<strong>' . htmlspecialchars($nivel) . ' (' . $numChamados . '):</strong><br>';



                                                // Agrupar atendimentos por técnico
                                                $tecnicos = [];
                                                foreach ($atendimentos as $atendimento) {
                                                    if (isset($atendimento['user_nome'], $atendimento['tarefa_id'])) {
                                                        $tecnicos[$atendimento['user_nome']][] = $atendimento['tarefa_id'];
                                                    }
                                                }

                                                echo '<ul style="margin: 0; padding-left: 15px;">'; // Ajustar margens e preenchimento da lista de tecnicos
                                                foreach ($tecnicos as $tecnico => $tarefa_ids) :
                                                    echo '<li style="margin-left: 1px; padding-left: 0px;">';
                                                    echo '<strong>' . htmlspecialchars($tecnico) . ':</strong>';

                                                    // Exibir o texto de empresa e tipo de tarefa ao passar o mouse
                                                    foreach ($tarefa_ids as $tarefaId) {
                                                        $tipoTexto = "Desconhecido";

                                                        // Encontrar o atendimento correspondente e definir o texto do tooltip
                                                        foreach ($atendimentos as $atendimento) {
                                                            if ($atendimento['tarefa_id'] == $tarefaId) {
                                                                $tipoTexto = $atendimento['tooltip_texto'] ?? "Desconhecido";
                                                                break;
                                                            }
                                                        }

                                                        echo '
                                                        <form action="atd.php" method="POST" id="form-atendimento-espera-' . htmlspecialchars($tarefaId) . '" style="display: inline;">
                                                            <input type="hidden" name="atd" value="' . htmlspecialchars($tarefaId) . '">
                                                            <a href="#" 
                                                            onclick="document.getElementById(\'form-atendimento-espera-' . htmlspecialchars($tarefaId) . '\').submit();" 
                                                            style="font-size: 1em; color: black; padding: 3px;" 
                                                            data-bs-toggle="tooltip" 
                                                            data-bs-placement="top" 
                                                            data-html="true"
                                                ...
                                                echo '</ul>'; // Fechar a lista de tecnicos
                                                echo '</div>';
                                            endforeach;
                                            ?>
                                        </div>
