...
function atd_projeto_detail_h($value)
{
  return htmlspecialchars((string)($value ?? ''), ENT_QUOTES, 'UTF-8');
}

function atd_projeto_detail_order_sql($ord, $dir)
{
  ...
  $map = [
    ...
    'forma' => 'tarefas.forma',
    'cliente' => 'tarefas.abertura',
  ];
  $key = array_key_exists((string)$ord, $map) ? (string)$ord : 'cliente';
  return $map[$key] . ' ' . $dir;
}

function atd_projeto_detail_status_info($status)
{
  $labels = [
...
function atd_projeto_detail_fetch_tasks(PDO $pdo, array $filters)
{
  ...
  $orderBy = atd_projeto_detail_order_sql($filters['ord'] ?? 'cliente', $filters['dir'] ?? 'ASC');
  ...
  $baseSql = "
    ...
    WHERE tarefas.id_projeto = :projeto
      AND tarefas.tecnico LIKE :tecnico
      AND tarefas.pessoa LIKE :solicitante
  ";
  ...
  $stmt = $pdo->prepare("SELECT tarefas.id as id_tarefa, tarefas.nome_tarefa, tarefas.dias, tarefas.desc_abertura, tarefas.abertura, tarefas.tecnico, tarefas.status,
      pessoas.pessoa_nom,
      categorias.cat_nome,
    ...
    " . $baseSql . "
    ORDER BY CASE WHEN tarefas.status = 4 THEN 1 ELSE 0 END ASC, " . $orderBy . "
...
function atd_projeto_detail_render_task_rows(array $rows)
{
  ob_start();
  foreach ($rows as $task) {
    $taskId = (int)$task['id_tarefa'];
    $taskStatus = (int)$task['status'];
    $statusInfo = atd_projeto_detail_status_info($taskStatus);
    $taskPercent = (int)$statusInfo['percent'];
    $taskTitle = trim((string)$task['nome_tarefa']) !== '' ? $task['nome_tarefa'] : strip_tags((string)$task['desc_abertura']);
    $taskOpenLabel = !empty($task['abertura']) ? date('d/m/y H:i', strtotime($task['abertura'])) : '-';
    $taskDeadlineLabel = (!empty($task['abertura']) && (int)$task['dias'] > 0) ? date('d/m/y H:i', strtotime($task['abertura'] . " +" . (int)$task['dias'] . " days")) : $taskOpenLabel;
    $taskTech = ((int)$task['tecnico'] === 0 || empty($task['tecnico_nome'])) ? 'Nao direcionado' : $task['tecnico_nome'];
    ?>
    <div class="projeto-task-row <?php echo $taskStatus === 4 ? 'is-done' : 'is-open'; ?>" data-task-card data-task-status="<?php echo $taskStatus; ?>">
      <div class="projeto-task-main">
        <strong><?php echo atd_projeto_detail_h($taskTitle); ?></strong>
        <div class="projeto-task-meta">
          Aberta em <?php echo atd_projeto_detail_h($taskOpenLabel); ?>
          <?php if (!empty($task['pessoa_nom'])) { ?> - Solicitante: <?php echo atd_projeto_detail_h($task['pessoa_nom']); ?><?php } ?>
        </div>
        <div class="projeto-task-badges">
          <?php if (!empty($task['cat_nome'])) { ?><span><?php echo atd_projeto_detail_h($task['cat_nome']); ?></span><?php } ?>
          <?php if (!empty($task['scat_nome'])) { ?><span><?php echo atd_projeto_detail_h($task['scat_nome']); ?></span><?php } ?>
          <?php if (!empty($task['itens_nome'])) { ?><span><?php echo atd_projeto_detail_h($task['itens_nome']); ?></span><?php } ?>
        </div>
      </div>
      <div class="projeto-task-owner">
        <span>Responsavel</span>
        <strong><?php echo atd_projeto_detail_h($taskTech); ?></strong>
      </div>
      <div class="projeto-task-date">
        <span>Prazo / Inicio</span>
        <strong><?php echo atd_projeto_detail_h($taskDeadlineLabel); ?></strong>
        <small>Inicio: <?php echo atd_projeto_detail_h($taskOpenLabel); ?></small>
      </div>
      ...
      <form action="tarefa.php" method="POST" class="projeto-task-action">
        ...
        <button type="submit" class="btn btn-light btn-sm projeto-open-task-btn"><i class="far fa-folder-open"></i> Ver</button>
      </form>
    </div>
    <?php
  }
...
function atd_projeto_detail_render_loader(array $pagination)
{
