...
function atd_3andar_h($value)
{
  return htmlspecialchars((string)($value ?? ''), ENT_QUOTES, 'UTF-8');
}

function atd_3andar_int($value, $default = 0)
{
  if ($value === null || $value === '') {
    return $default;
  }
...
function atd_3andar_statuses($statusFilter)
{
  ...
  $status = atd_3andar_int($statusFilter, 11);
...
function atd_3andar_collect_filters(array $source, $type = 'tasks')
{
  $status = $source['f_sts'] ?? 11;
  $page = max(1, atd_3andar_int($source['page'] ?? 1, 1));
  $perPage = atd_3andar_int($source['per_page'] ?? 30, 30);
  $perPage = min(max($perPage, 10), 100);

  $filters = [
    'type' => $type,
    'f_sts' => (string)$status,
    'statuses' => atd_3andar_statuses($status),
    'f_clt' => atd_3andar_int($source['f_clt'] ?? 0),
    'f_sol' => atd_3andar_int($source['f_sol'] ?? 0),
  ...
  $order = atd_3andar_normalize_order($source['ord'] ?? 'status', $source['order_dir'] ?? 'ASC', $type);
...
function atd_3andar_company_filter($column, array &$params)
{
...
function atd_3andar_build_where($alias, array $filters, array &$params, $isTask = false)
{
  $where = [];
  $statusHolders = [];
  ...
  if ($isTask && $filters['f_palavra'] !== '') {
    $where[] = '(LOWER(tarefas_terc_andar.nome_tarefa) LIKE LOWER(:f_palavra)
      OR LOWER(tarefas_terc_andar.desc_abertura) LIKE LOWER(:f_palavra)
      OR LOWER(tarefas_terc_andar.desc_fechamento) LIKE LOWER(:f_palavra)
  ...
  if ($isTask && $filters['data_1'] !== '' && $filters['data_2'] !== '') {
    $where[] = 'tarefas_terc_andar.abertura BETWEEN :data_1 AND :data_2';
...
function atd_3andar_bind_params(PDOStatement $stmt, array $params)
{
...
function atd_3andar_fetch_tasks(PDO $pdo, array $filters)
{
  $params = [];
  $where = atd_3andar_build_where('tarefas_terc_andar', $filters, $params, true);
  $where .= atd_3andar_company_filter('clientes.clt_id', $params);

  $baseJoins = "
    FROM tarefas_terc_andar
    INNER JOIN clientes ON tarefas_terc_andar.cliente = clientes.clt_id
    LEFT JOIN pessoas ON tarefas_terc_andar.pessoa = pessoas.pessoa_id
    LEFT JOIN locais ON tarefas_terc_andar.local = locais.local_id

    LEFT JOIN categorias_terc_andar AS categorias 
      ON tarefas_terc_andar.categoria = categorias.id

    LEFT JOIN subcategorias_terc_andar AS subcategorias 
      ON tarefas_terc_andar.subcategoria = subcategorias.id

    LEFT JOIN tipos_terc_andar AS tipos
      ON tarefas_terc_andar.tipo = tipos.id

    LEFT JOIN niveis_terc_andar AS niveis
      ON tarefas_terc_andar.nivel = niveis.id

    LEFT JOIN itens ON tarefas_terc_andar.item = itens.itens_id
    LEFT JOIN usuarios AS tecnico ON tarefas_terc_andar.tecnico = tecnico.user_id
  ";
  ...
  $dataJoins = $baseJoins . "
    ...
    ) espera_total ON espera_total.espera_tarefa = tarefas_terc_andar.id
    LEFT JOIN (
      SELECT inter_tarefa, MAX(inter_data) AS ultima_interacao
      FROM inter_terc_andar
      WHERE inter_tipo > 0
      GROUP BY inter_tarefa
    ) inter_total ON inter_total.inter_tarefa = tarefas_terc_andar.id
  ";

  $countSql = "SELECT COUNT(*) " . $baseJoins . $where;
  $count = $pdo->prepare($countSql);
  atd_3andar_bind_params($count, $params);
  $count->execute();
  ...
  $sql = "SELECT
    ...
    " . $dataJoins . $where . "
    ORDER BY " . $filters['order_sql'] . ", tarefas_terc_andar.id DESC
    LIMIT :limit OFFSET :offset";

  $stmt = $pdo->prepare($sql);
  atd_3andar_bind_params($stmt, $params);
  ...
  return [
    ...
    'pagination' => atd_3andar_pagination($total, $filters),
  ];
}

function atd_3andar_pagination($total, array $filters)
{
...
function atd_3andar_status_badge($status)
{
  $map = [
...
function atd_3andar_nivel_badge($nivel)
{
  if ((int)$nivel <= 0) {
...
function atd_3andar_forma_nome($forma)
{
  $map = [
    1 => 'Remoto',
    2 => 'Presencial',
...
function atd_3andar_desc($text, $limit = 180)
{
...
function atd_3andar_sort_button($key, $label, array $filters)
{
  ...
  return '<button type="button" class="btn btn-link btn-sm p-0 projeto-sort" data-projeto-sort="' . atd_3andar_h($key) . '" data-projeto-dir="' . $nextDir . '"><i class="fas ' . $icon . '"></i> ' . atd_3andar_h($label) . '</button>';
}

function atd_3andar_render_tasks_table(array $rows, array $filters, array $pagination)
{
  ob_start();
  ?>
  <div class="projeto-list-shell" data-list-kind="tasks">
    ...
    <div class="projeto-table-wrap">
      <table class="table table-hover small projeto-table">
        <thead>
          <tr>
            <th><?php echo atd_3andar_sort_button('id', 'ID', $filters); ?></th>
            <th><?php echo atd_3andar_sort_button('cliente', 'Cliente', $filters); ?></th>
            <th>Tarefa</th>
            <th><?php echo atd_3andar_sort_button('abertura', 'Abertura', $filters); ?></th>
            <th>Categoria</th>
            <th><?php echo atd_3andar_sort_button('nivel', 'Nivel', $filters); ?></th>
            <th><?php echo atd_3andar_sort_button('tecnico', 'Tecnico', $filters); ?></th>
            <th><?php echo atd_3andar_sort_button('status', 'Status', $filters); ?></th>
          </tr>
        </thead>
        <tbody>
          <?php echo atd_3andar_render_task_rows($rows); ?>
        </tbody>
      </table>
    </div>
    <?php echo atd_3andar_render_infinite_loader($pagination, 'tarefas'); ?>
  </div>
  <?php
  return ob_get_clean();
}

function atd_3andar_render_task_rows(array $rows)
{
  ob_start();
  if (!$rows) { ?>
    <tr>
  ...
  <?php foreach ($rows as $row) { ?>
    <tr class="projeto-open-row" data-projeto-open-url="tarefa.php?tarefa=<?php echo (int)$row['id']; ?>">
      ...
      <td class="align-middle">
        <strong><?php echo atd_3andar_h($row['clt_nomef']); ?></strong><br>
        <span class="text-muted"><i class="far fa-user"></i> <?php echo atd_3andar_h($row['pessoa_nom']); ?></span>
      </td>
      <td class="align-middle projeto-desc-cell">
        <div class="projeto-project-summary">
          <strong class="projeto-project-title"><?php echo atd_3andar_h(atd_3andar_desc($row['nome_tarefa'], 75)); ?></strong>
          <span class="text-muted projeto-project-description"><?php echo atd_3andar_h(atd_3andar_desc($row['desc_abertura'] ?? '', 95)); ?></span>
        </div>
      </td>
      <td class="align-middle"><?php echo $row['abertura'] ? date('d/m/y H:i', strtotime($row['abertura'])) : '-'; ?></td>
      <td class="align-middle">
        <?php echo atd_3andar_h($row['cat_nome']); ?><br>
        <span class="text-muted"><?php echo atd_3andar_h($row['scat_nome']); ?></span>
      </td>
      <td class="align-middle"><?php echo atd_3andar_nivel_badge($row['nivel']); ?></td>
      <td class="align-middle"><?php echo atd_3andar_h($row['tecnico_nome'] ?: 'Nao direcionado'); ?></td>
      <td class="align-middle"><?php echo atd_3andar_status_badge($row['status']); ?></td>
    </tr>
  <?php }
  return ob_get_clean();
}

function atd_3andar_render_pagination(array $pagination)
{
  ob_start();
  ?>
...
function atd_3andar_render_infinite_loader(array $pagination, $label = 'registros')
{
  ...
  <div class="projeto-infinite-loader" data-projeto-infinite-loader data-next-page="<?php echo $hasMore ? (int)$pagination['next_page'] : ''; ?>" data-has-more="<?php echo $hasMore ? '1' : '0'; ?>">
    <?php if ($hasMore) { ?>
      ...
      <span>Role para carregar mais <?php echo atd_3andar_h($label); ?></span>
    <?php } else { ?>
      <span><?php echo atd_3andar_h($doneText); ?></span>
    <?php } ?>
  </div>
  <?php
  return ob_get_clean();
