...
if (!empty($dataFiltro) && !empty($dataFiltro2)) {
    $whereClauses[] = "(
        DATE(t.dateadded) BETWEEN :data_1 AND :data_2
        OR DATE(t.datefinished) BETWEEN :data_1 AND :data_2
    )";
    $params[':data_1'] = $dataFiltro;
    $params[':data_2'] = $dataFiltro2;
} elseif (!empty($dataFiltro)) {
    $whereClauses[] = "(
        DATE(t.dateadded) = :data_1
        OR DATE(t.datefinished) = :data_1
    )";
    $params[':data_1'] = $dataFiltro;
}
...
$colunas_validas = [
    ...
    'status' => 'ts.name', // <- essa linha resolve o problema da ordenação
    'inicio' => 't.dateadded',
    'prazo' => 't.duedate',
...
$ord = $_POST['ord'] ?? 'id';
$order_dir = strtoupper($_POST['order_dir'] ?? 'DESC');

$order_dir = in_array($order_dir, ['ASC', 'DESC']) ? $order_dir : 'DESC';
$order_by = $colunas_validas[$ord] ?? '';


$order_dir_validas = ['ASC', 'DESC'];

if (!in_array($ord, $colunas_validas)) {
    $ord = 'id';
}
...
$query = "
    SELECT t.id, t.name, t.description, c.userid AS id_cliente, c.company AS nome_cliente,
           t.priority, t.status, ts.name AS status, cfv.value AS total_artes,
...
<body>
                        ...
                        <form action="#" method="POST">
                            <div class="form-row align-items-center">
                                ...
                                <div class="col-2 col-form-label-sm">
                                    <label class="my-0">Título:</label>
                                    <input type="text" name="titulo" class="form-control form-control-sm my-1" value="<?= isset($_POST['titulo']) ? htmlspecialchars($_POST['titulo']) : '' ?>">
                                </div>
                                    ...
                                    <div class="dropdown" style="width: 190px">
                                        ...
                                        <div class="dropdown-menu" style="padding: 10px; width: 190px; border-radius: 4px;">
                                            ...
                                            <?php foreach ($todosStatus as $status) : ?>
                                                <div class="form-check">
                                                    <input class="form-check-input" type="checkbox" name="status[]" value="<?= $status['id'] ?>" id="status<?= $status['id'] ?>" <?php echo (isset($_POST['status']) && in_array($status['id'], $_POST['status'])) ? 'checked' : ''; ?>>
                                ...
                                <div class="col-auto col-form-label-sm">
                                    <label class="my-0">Tipo de Data:</label>
                                    <select class="form-control form-control-sm" name="typeDataFiltro" id="typeDataFiltro">
                                        <option value="1" <?= (isset($_POST['typeDataFiltro']) && $_POST['typeDataFiltro'] == '1') ? 'selected' : '' ?>>Todas</option>
                                        <option value="2" <?= (isset($_POST['typeDataFiltro']) && $_POST['typeDataFiltro'] == '2') ? 'selected' : '' ?>>Criação</option>
                                        <option value="3" <?= (isset($_POST['typeDataFiltro']) && $_POST['typeDataFiltro'] == '3') ? 'selected' : '' ?>>Prazo</option>
                                        <option value="4" <?= (isset($_POST['typeDataFiltro']) && $_POST['typeDataFiltro'] == '4') ? 'selected' : '' ?>>Finalizado</option>
                                    </select>
                                </div>


                                <div class="col-1 col-form-label-sm">
                                    <label class="my-0">Data Inicio:</label>
                                    <input id="data_1" name="data_1" type="date" placeholder="dd/mm/aaaa" class="form-control form-control-sm my-1" value="<?= isset($_POST['data_1']) ? htmlspecialchars($_POST['data_1']) : '' ?>">
                                </div>

                                <div class="col-1 col-form-label-sm">
                                    <label class="my-0">Data Fim:</label>
                                    <input id="data_2" name="data_2" type="date" placeholder="dd/mm/aaaa" class="form-control form-control-sm my-1" value="<?= isset($_POST['data_2']) ? htmlspecialchars($_POST['data_2']) : '' ?>">
                                </div>


                                <button type="submit" name="filtrar" class="btn btn-sm btn-info my-1 mr-3 ml-3 mt-4">Filtrar</button>

                                <button type="button" class="btn btn-sm btn-outline-secondary my-1 mt-4" onclick="limparFiltros()">Limpar</button>


                                <div class="col-auto">
                                    <a href="./home.php" class="btn btn-sm btn-outline-info my-1 mt-4">
    ...
    <script>
        function toggleAllStatuses() {
            let selectAll = document.getElementById("select-all-status");
