...
| catalog | `catlg` | `nivel3` | api-read | Native NestJS read paths now cover list, detail, filter metadata and contextual resolution by client/category with TI/DevOps permission scoping. Legacy PHP remains for write/UI until the next migration stages. See `docs/CATALOG-MIGRATION.md`. |
