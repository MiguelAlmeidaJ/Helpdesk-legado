...
| assets | retired `ativos/` | `plugins_app`, `patrimonios` | decommissioned | Legacy runtime and residual integrations removed. Final gate: `pnpm legacy:assets:verify`. Database data remains preserved and no 1:1 Nest/Next migration is planned. See `docs/ASSETS-DECOMMISSION.md`. |
| improvements | retired `melhorias/` | `nivel3` (`melhorias`, `interatividade_melhorias`) | decommissioned | Operational PHP removed after usage/data audit. Historical tables are preserved; native analytics still reads `melhorias`. No 1:1 Nest/Next migration is planned. See `docs/IMPROVEMENTS-DECOMMISSION.md`. |
| catalog | retired `catlg/` | `nivel3` | retired | Native NestJS read/write paths, `/catalog`, `/catalog/check` and ticket-context resolution are authoritative with TI/DevOps permission scoping. Final gate: `pnpm legacy:catalog:verify`. See `docs/CATALOG-MIGRATION.md`. |
