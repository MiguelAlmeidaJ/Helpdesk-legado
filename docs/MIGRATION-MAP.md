...
| catalog | `catlg` | `nivel3` | inventory | Five legacy PHP entry points are being inventoried before native migration. `localizar_catalogo.php` couples catalog resolution to ticket context, so `catlg/` can only be retired after Nest/Next cutover. See `docs/CATALOG-MIGRATION.md`. |
