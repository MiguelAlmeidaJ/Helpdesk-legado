...
| reports | `rel` | `nivel3` | web | `0045a` starts the native reports context with ticket totals by client; `0045b` adds totals by technician and extracts shared report visibility. Legacy report PHP remains only until native PDF parity allows each migrated entry point to be retired safely. |
