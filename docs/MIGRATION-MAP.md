...
| catalog | `catlg` | `nivel3` | web | Native NestJS read/write paths and the Next.js catalog UI are available with TI/DevOps permission scoping. The legacy module has no delete flow; `catlg/` remains until the tickets integration and cutover are complete. See `docs/CATALOG-MIGRATION.md`. |
