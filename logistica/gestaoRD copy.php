...
$stmtAprovado2 = $pdo->prepare("SELECT IFNULL(SUM(amount), 0) FROM running_balance r WHERE r.status = 2 AND r.aj = 1");
$stmtAprovado2->execute($params);
$totalAprovado2 = $stmtAprovado->fetchColumn();
...
$stmtCategorias = $pdo->prepare("SELECT c.categories, SUM(r.amount) as balance FROM running_balance r INNER JOIN category c ON c.id = r.category_id WHERE r.status IN (4) AND r.aj = 1 $filtroData GROUP By c.categories ORDER BY balance DESC");
$stmtCategorias->execute($params);
$categoriasResumo = $stmtCategorias->fetchAll(PDO::FETCH_ASSOC);

$stmtEmpresas = $pdo->prepare("SELECT r.cliente, SUM(r.amount) as balance FROM running_balance r WHERE r.status IN (4) AND r.aj = 1 $filtroData GROUP BY r.cliente ORDER BY balance DESC");
$stmtEmpresas->execute($params);
$empresasResumo = $stmtEmpresas->fetchAll(PDO::FETCH_ASSOC);

$stmt = $pdo->prepare("SELECT u.id as user_id, u.firstname, u.lastname, SUM(r.amount) AS balance FROM running_balance r JOIN user u ON u.id = r.user_id WHERE r.status IN (4) AND r.aj = 1 $filtroData GROUP BY u.id ORDER BY balance DESC");
$stmt->execute($params);
...
<body>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Substitua o seu código da matriz expansível por este
