...
try {
    ...
    if ($action === 'toggle_status') {
        ...
        $tabelasPermitidas = ['categorias_grupo', 'categorias_subgrupo', 'categorias_classificacao', 'categorias_tipo_documento', 'cads_forma_pag'];
        ...
        <div class="card">
            <div class="card-header py-2">
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="m-0 font-weight-bold">Gestão de Cadastros Financeiros</h5>
                                ...
                                <tbody>
                                    <?php foreach ($pagamentos as $item) : ?>
                                        <tr>
                                            ...
                                            <td class="text-center"><?= $item['status'] == 1 ? '<span class="badge badge-success">Ativo</span>' : '<span class="badge badge-secondary">Inativo</span>' ?></td>
                                            <td class="text-center">
                                                <form method="POST" style="display: inline;"><input type="hidden" name="action" value="toggle_status"><input type="hidden" name="id" value="<?= $item['id'] ?>"><input type="hidden" name="tabela" value="cads_forma_pag"><input type="hidden" name="tabRedirect" value="pagamentos">
                                                    <button type="submit" class="btn btn-sm <?= $item['status'] ? 'btn-secondary' : 'btn-success' ?>" title="<?= $item['status'] ? 'Desativar' : 'Ativar' ?>"><i class="fas <?= $item['status'] ? 'fa-power-off' : 'fa-check-circle' ?>"></i></button>
                                                </form>
