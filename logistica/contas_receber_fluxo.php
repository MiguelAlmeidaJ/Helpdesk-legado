...
if ($action === 'excluir_recebimento') {
    try {
        ...
        $stmt = $pdo->prepare("SELECT id_conta_receber FROM recebimentos WHERE id = ?");
...
if ($action === 'add_conta_receber' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($_POST['descricao']) || empty($_POST['valor_total']) || empty($_POST['data_vencimento']) || empty($_POST['id_cliente']) || empty($_POST['id_grupo']) || empty($_POST['id_subgrupo']) || empty($_POST['id_classificacao'])) {
        ...
        header("Location: contas_receber_fluxo.php");
        exit;
    }
...
if ($action === 'excluir_conta_simples' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    // DEBUG: Registra no log do servidor (ex: error_log do Apache/XAMPP)
    error_log("Tentativa de exclusáo - Action: $action");

    $id = filter_input(INPUT_POST, 'id', FILTER_VALIDATE_INT);

    // DEBUG: Registra no log do servidor (ex: error_log do Apache/XAMPP)
    error_log("Tentativa de exclusáo - Action: $action | ID recebido: " . ($_POST['id'] ?? 'vazio') . " | ID Validado: " . ($id ?: 'falhou'));

    if ($id) {
        $stmtCheck = $pdo->prepare("SELECT COUNT(*) FROM recebimentos WHERE id_conta_receber = ?");
        $stmtCheck->execute([$id]);
        $temRecebimento = $stmtCheck->fetchColumn();

        if ($temRecebimento > 0) {
            error_log("Falha: Conta $id possui $temRecebimento recebimentos.");
            $_SESSION['mensagem_erro'] = 'Não à possível excluir uma fatura que já possui recebimentos.';
        } else {
            $pdo->beginTransaction();
            try {
                $delDivisao = $pdo->prepare("DELETE FROM contas_receber_divisao WHERE id_conta_receber = ?");
                $delDivisao->execute([$id]);

                $delConta = $pdo->prepare("DELETE FROM contas_receber WHERE id = ?");
                $delConta->execute([$id]);

                $pdo->commit();
                error_log("Sucesso: Conta $id e suas divisões foram excluídas.");
                $_SESSION['mensagem_sucesso'] = 'Conta a receber excluída com sucesso!';
            } catch (Exception $e) {
                $pdo->rollBack();
                error_log("Erro PDO na conta $id: " . $e->getMessage());
                $_SESSION['mensagem_erro'] = 'Erro ao excluir conta: ' . $e->getMessage();
            }
        }
    } else {
        // Se cair aqui, o ID enviado no formulário não é um número inteiro válido ou o campo 'name' não é 'id'
        error_log("Erro: ID inválido ou não enviado via POST.");
        $_SESSION['mensagem_erro'] = 'ID da conta inválido para exclusáo.';
    }
...
// if ($action === 'excluir_conta_com_recorrencia' && $_SERVER['REQUEST_METHOD'] === 'POST') {
//     $id = filter_input(INPUT_POST, 'id', FILTER_VALIDATE_INT);
//     $excluirRecorrencia = isset($_POST['excluir_recorrencia']) ? 'SIM' : 'NºO';
//     $rawId = $_POST['id'] ?? 'Nulo/Vazio';

//     // Script para injetar no navegador antes de redirecionar
//     echo "<script>
//         console.group('?? DEBUG EXCLUSÃO RECORRENTE');
//         console.log('Ação: $action');
//         console.log('ID Bruto (POST): $rawId');
//         console.log('ID Validado (INT): " . ($id ?: 'ERRO DE VALIDAÇÃO') . "');
//         console.log('Excluir Regra de Recorrência? $excluirRecorrencia');
//     </script>";
...
//         if ($temRecebimento > 0) {
//             echo "<script>console.error('? Bloqueado: Conta possui $temRecebimento recebimentos.');</script>";
//             $_SESSION['mensagem_erro'] = 'Não à possível excluir uma fatura que já possui recebimentos.';
//         } else {
//             $pdo->beginTransaction();
//             try {
...
//                 if ($fatura) {
//                     echo "<script>console.log('? Fatura encontrada: Cliente {$fatura['id_cliente']} | {$fatura['descricao']}');</script>";
//                 }

//                 $pdo->prepare("DELETE FROM contas_receber_divisao WHERE id_conta_receber = ?")->execute([$id]);
//                 $pdo->prepare("DELETE FROM contas_receber WHERE id = ?")->execute([$id]);

//                 if ($excluirRecorrencia === 'SIM' && $fatura) {
//                     $sqlDelRec = "DELETE FROM recorrencias WHERE tipo = 'Receber' AND id_cliente = ? AND descricao_padrao = ?";
//                     $pdo->prepare($sqlDelRec)->execute([$fatura['id_cliente'], $fatura['descricao']]);
//                     echo "<script>console.log('??? Regra de Recorrência deletada.');</script>";
//                 }

//                 $pdo->commit();
//                 echo "<script>console.log('?? Transação concluída com sucesso!');</script>";
//                 $_SESSION['mensagem_sucesso'] = 'Operação de exclusáo realizada com sucesso!';
//             } catch (Exception $e) {
//                 $pdo->rollBack();
//                 echo "<script>console.error('?? Erro PDO: " . addslashes($e->getMessage()) . "');</script>";
//                 $_SESSION['mensagem_erro'] = 'Erro ao excluir conta: ' . $e->getMessage();
//             }
//         }
//     }

//     // Pausa o PHP para você conseguir ler o console antes do redirecionamento
//     echo "<script>
//         console.log('Redirecionando em 5 segundos...');
//         console.groupEnd();
//         setTimeout(() => { window.location.href = 'contas_receber_fluxo.php'; }, 5000);
//     </script>";
//     exit;
// }
...
if (!empty($filtro_data_inicio)) {
    $whereConditions[] = "COALESCE(r.data_recebimento, cr.data_vencimento) >= :data_inicio";
    $params[':data_inicio'] = $filtro_data_inicio;
}

if (!empty($filtro_data_fim)) {
    $whereConditions[] = "COALESCE(r.data_recebimento, cr.data_vencimento) <= :data_fim";
...
$colunasPermitidas = ['nome_cliente', 'descricao', 'data_vencimento', 'valor_total', 'data_recebimento', 'total_recebido', 'saldo', 'status_id'];
if (!in_array($orderBy, $colunasPermitidas)) $orderBy = 'data_recebimento';
...
LEFT JOIN (
    SELECT 
        id_conta_receber,
        MAX(data_recebimento) AS data_recebimento,
        SUM(valor_recebido) AS total_recebido,
        SUBSTRING_INDEX(
            GROUP_CONCAT(observacao ORDER BY data_recebimento DESC SEPARATOR '||'),
            '||',
            1
        ) AS observacao
    FROM recebimentos
    GROUP BY id_conta_receber
) r ON r.id_conta_receber = cr.id
";
...
// $sql .= " ORDER BY ($orderBy IS NULL) ASC, $orderBy $orderDir";
...
$sqlResumo = "
    SELECT 
        SUM(
            CASE 
                WHEN cr.data_vencimento BETWEEN :data_inicio AND :data_fim
                THEN cr.valor_total
                ELSE 0
            END
        ) AS total_faturado,
        ...
        SUM(
            CASE 
                WHEN r.data_recebimento BETWEEN :data_inicio AND :data_fim
                THEN r.total_recebido
                ELSE 0
            END
        ) AS total_recebido,
...
$totalRecebidoFiltrado = $resumoFiltro['total_recebido'] ?? 0;
...
<body>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-2 align-self-end">
                                    <div class="btn-group w-100">
                                        ...
                                        <a href="contas_receber_fluxo.php" class="btn btn-outline-secondary btn-sm" title="Limpar Filtros"><i class="fas fa-eraser"></i></a>
                                    </div>
                                </div>
                            </div>
                        </form>
                    ...
                    <table class="table table-sm table-bordered table-hover contas-rec-table">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center" style="width: 130px;"><?= sortLink('Dt. Vencimento', 'data_vencimento', $orderBy, $orderDir) ?></th>
                                <th><?= sortLink('Valor Total', 'valor_total', $orderBy, $orderDir) ?></th>
                                <th style="width: 140px;"><?= sortLink('Dt. Recebimento', 'data_recebimento', $orderBy, $orderDir) ?></th>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Excluir recebimento
            $(document).on('click', '.btn-excluir-recebimento', function() {
