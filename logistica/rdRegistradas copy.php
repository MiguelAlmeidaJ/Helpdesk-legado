...
$stmtCategorias = $pdo->prepare("SELECT c.categories, SUM(r.amount) as balance FROM running_balance r INNER JOIN category c ON c.id = r.category_id WHERE r.status IN (1) AND r.aj = 1 $filtroData GROUP By c.categories ORDER BY balance DESC");
$stmtCategorias->execute($params);
$categoriasResumo = $stmtCategorias->fetchAll(PDO::FETCH_ASSOC);

$stmtEmpresas = $pdo->prepare("SELECT r.cliente, SUM(r.amount) as balance FROM running_balance r WHERE r.status IN (1) AND r.aj = 1 $filtroData GROUP BY r.cliente ORDER BY balance DESC");
$stmtEmpresas->execute($params);
$empresasResumo = $stmtEmpresas->fetchAll(PDO::FETCH_ASSOC);

$stmt = $pdo->prepare("SELECT u.firstname, u.lastname, SUM(r.amount) AS balance FROM running_balance r JOIN user u ON u.id = r.user_id WHERE r.status IN (1) AND r.aj = 1 $filtroData GROUP BY u.id ORDER BY balance DESC");
$stmt->execute($params);
            ...
            <div class="card-body">
                <div class="container-fluid">
                    ...
                    <div class="text-center m-0 p-0">
                        <hr class="m-1">
                        <h4 class="m-0 p-0">Registradas Aguardando Aprovação</h4>
                        <hr class="m-1 mb-3">
                    </div>
