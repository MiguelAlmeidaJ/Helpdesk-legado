...
if ($dataInicio < $dataCorteCategorias) {
    ...
    $sqlAntigo = "SELECT c.categories, SUM(r.amount) as balance 
                  ...
                  WHERE r.status = 1 AND r.aj = 1 AND r.date_created BETWEEN :inicio AND :fim
...
// Resumo por Cliente 
$stmtEmpresas = $pdo->prepare("SELECT r.cliente, SUM(r.amount) as balance FROM running_balance r WHERE r.status = 1 AND r.aj = 1 $filtroData GROUP BY r.cliente ORDER BY balance DESC");
...
$stmtUsuarios = $pdo->prepare("SELECT u.user_id, u.user_nome, SUM(r.amount) AS balance FROM running_balance r left JOIN usuarios u ON u.user_id = r.user_id WHERE r.status = 1 AND r.aj = 1 $filtroData GROUP BY u.user_id, u.user_nome ORDER BY balance DESC");
