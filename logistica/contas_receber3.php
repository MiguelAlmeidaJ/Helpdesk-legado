...
$colunasPermitidas = ['nome_cliente', 'descricao', 'data_vencimento', 'valor_total', 'saldo', 'status_id'];
...
LEFT JOIN (
    SELECT 
        ...
        MAX(data_recebimento) AS ultima_data_recebimento
    FROM recebimentos
    GROUP BY id_conta_receber
) AS r ON cr.id = r.id_conta_receber
LEFT JOIN contas_receber_divisao AS crd ON cr.id = crd.id_conta_receber
LEFT JOIN unidade_negocio AS un ON cr.unidade_negocio = un.id
LEFT JOIN status_contas AS s ON cr.status_id = s.id
";
...
$contasInadimplentesArrGlobal = [];

$mesAtual = (int)date('m');
$anoAtual = (int)date('Y');
$hoje = new DateTime(); // Para calcular a diferença de dias com precisão

foreach ($contas_modal_detalhe as $item) {
    $sId = (int)$item['status_id'];
    $dataVencStr = $item['data_vencimento'];
    $saldo = (float)($item['saldo'] ?? 0);

    $vencimento = new DateTime($dataVencStr);
    $mesVenc = (int)$vencimento->format('m');
    $anoVenc = (int)$vencimento->format('Y');

    // 1. Regra de Inadimplência (Vencida em mês anterior)
    $isMesAnterior = ($anoVenc < $anoAtual || ($anoVenc == $anoAtual && $mesVenc < $mesAtual));

    if ($saldo > 0.01 && $sId !== 3) { // 3 = Recebido (ignoramos)

        if ($isMesAnterior || $sId === 5) {
            // Se venceu em mês anterior ou já está marcada como Inadimplente
            $contasInadimplentesArrGlobal[] = $item;
        } else {
            // Se é do mês atual, cai na lista de "Pendentes/A Vencer"
                            ...
                            <?php if (empty($contas_a_receber)) : ?>
                                ...
                                <?php else : foreach ($contas_a_receber as $item) : ?>
                                    <tr class="<?=
                                        ...
                                        <td><?= $item['ultima_data_recebimento'] ? date('d/m/Y', strtotime($item['ultima_data_recebimento'])) : '' ?></td>
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-8">
                            ...
                            $contasPendentesFiltradas = [];
                            foreach ($contas_modal_detalhe as $item) {
                                if ((float)$item['saldo'] > 0.01 && (int)$item['status_id'] !== 3) {
                                    $contasPendentesFiltradas[] = $item;
                                }
                            }

                            // 2. Inadimplentes: BUSCA GLOBAL (Ignora o filtro de data para mostrar o rombo real)
                            $sqlInadTotal = "
                                ...
                                AND (
                                    cr.status_id = 5 OR 
                                    (cr.data_vencimento < DATE_FORMAT(CURRENT_DATE, '%Y-%m-01'))
                                )
                                ORDER BY cr.data_vencimento ASC
                            ";
                            $contasInadimplentesArrGlobal = $pdo->query($sqlInadTotal)->fetchAll(PDO::FETCH_ASSOC);

                            // Atualiza o contador do Card para bater com a lista nova
                            $totalInadimplenteGlobal = array_sum(array_column($contasInadimplentesArrGlobal, 'saldo'));
                            $contasInadimplentesGlobal = count($contasInadimplentesArrGlobal);
                            ...
                            function renderTabelaContas($contas)
                            {
                                ...
                                foreach ($contas as $item) {
                                    ...
                                    $diasAtraso = $intervalo->invert ? $intervalo->days : 0; // Só conta se for no passado

                                    // Lógica Visual baseada nos seus IDs
                                    switch ($sId) {
                                        case 2:
                                        ...
                                        case 5:
                                            ...
                                            break;
                                        default:
                                            // Se o ID for 1 (A vencer) ou 4 (Vencido), validamos pelos 5 dias
                                            if ($diasAtraso > 5) {
                            ...
                            ?>
                                    <tr>
                                        ...
                                        <td class="text-right"><?= $dataVenc->format('d/m/Y') ?></td>
