...
if ($action === 'excluir_recebimento') {
    try {
        ...
        if (!$conta) {
            throw new Exception('Conta não encontrada na tabela contas_receber.');
        }
        ...
        // 4?? Somar recebimentos restantes
        ...
        // 5?? Aplicar regra unificada de status
        ...
        $stmt = $pdo->prepare("
            ...
            WHERE id = ?
        ");
        $stmt->execute([$saldo, $novo_status, $id_conta]);
                                ...
                                <?php else : foreach ($contas_a_receber as $item) : ?>
                                    <tr class="<?= $item['status'] == 'Recebido' ? 'table-recebido' : ($item['status'] == 'Parcialmente Recebido' ? 'table-parcial' : '') ?>">
                                        ...
                                        <td class="text-center">
                                            ...
                                            $isVencido = strtotime($dataVenc) < time() && $status != 'Recebido';

                                            // Determinar se é inadimplente (mês anterior ao atual e ainda não recebido)
                                            $isInadimplente = $status != 'Recebido'
                                                && ($anoVenc < $anoAtual || ($anoVenc == $anoAtual && $mesVenc < $mesAtual));
                                        ...
                                        <td class="d-flex justify-content-end gap-1 flex-nowrap">
                                            ...
                                            <button type="button" class="btn btn-sm btn-info btn-exibir-recebimentos" data-toggle="modal" data-target="#modalExibirRecebimentos" data-id="<?= $item['fatura_id'] ?>" title="Exibir Recebimentos">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.btn-exibir-recebimentos').on('click', function() {
                ...
                $.post('contas_receber.php', {
                    ...
                    <tr data-id="${rec.id_recebimento}">
                        ...
                        <td>
                            <input type="text" class="form-control form-control-sm agencia" value="${rec.ag_nome || ''}">
                        </td>
                        <td>
                            <input type="text" class="form-control form-control-sm observacao" value="${rec.observacao || ''}">
                        </td>
                        <td>
            ...
            $(document).on('click', '.btn-salvar-recebimento', function() {
                ...
                const agencia = row.find('.agencia').val();
