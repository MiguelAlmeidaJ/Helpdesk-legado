...
<body>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <?php
                                            $anexos = json_decode($rd['anexos'], true);
                                            ...
                                            } elseif ($rd['category_id'] == 43) { // Supondo que 43 é "Nota de Serviço PJ"
                                                // Se NºO tem anexos E é categoria 43, exibe o alerta
                                                echo '<span class="badge badge-danger p-1" title="Nota de Serviço obrigatória não enviada!">';
                                                echo '  <i class="fas fa-exclamation-triangle"></i> NOTA PENDENTE';
                                                echo '</span>';
                                            } else {
                                                // Se não for categoria 43 e não tiver anexos
                                        ...
                                        <td class="text-center">
                                            <?php if ($rd['status'] == 1) : ?>
                                                <button type="button" class="btn btn-warning btn-sm edit-btn" data-toggle="modal" data-target="#editModal" data-id="<?= $rd['id'] ?>" data-valor="<?= $rd['amount'] ?>" data-nome="<?= htmlspecialchars($rd['user_nome']) ?>" data-categoria="<?= $rd['category_id'] ?? '' ?>" data-cliente_id="<?= $rd['clt_id'] ?? '' ?>" data-observacoes="<?= htmlspecialchars($rd['remarks']) ?>" data-pix_type="<?= $rd['pix_type'] ?? '' ?>" data-pix="<?= htmlspecialchars($rd['pix'] ?? '') ?>" data-anexos="<?= htmlspecialchars($rd['anexos'] ?? '[]') ?>">
                                            ...
                                            <?php endif; ?>
                                            <button type="button" class="btn btn-info btn-sm duplicar-btn" data-toggle="modal" data-target="#duplicarModal" data-id="<?= $rd['id'] ?>" data-valor="<?= $rd['amount'] ?>" data-categoria="<?= $rd['category_id'] ?? '' ?>" data-pix_type="<?= $rd['pix_type'] ?? '' ?>" data-pix="<?= $rd['pix'] ?? '' ?>" data-cliente_id="<?= $rd['clt_id'] ?? '' ?>" data-observacoes="<?= $rd['remarks'] ?>" title="Duplicar RD">
        ...
        <div class="modal-dialog" role="document">
            <form class="modal-content" action="addDespesa.php" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <hr>
                    <div class="form-group esconde-secao">
                        <label for="pdfFileInput_add">Anexar Comprovante(s) (PDF)</label>
                        <input type="file" id="pdfFileInput_add" class="form-control-file auto-upload esconde-secao" accept="application/pdf" multiple data-status-div="uploadStatus_add" data-remarks-id="remarks" data-anexos-input="anexos_json_add">
            ...
            <form class="modal-content" action="editarRD.php" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="file" id="pdfFileInput_edit" class="form-control-file auto-upload" accept="application/pdf" multiple data-status-div="uploadStatus_edit" data-remarks-id="edit_remarks" data-anexos-input="anexos_novos_json_edit">
    ...
    <div class="modal fade" id="duplicarModal" tabindex="-1">
        <div class="modal-dialog" role="document">
            <form class="modal-content" action="addDespesa.php" method="POST">
                <div class="modal-header">
                    <h5 class="modal-title text-primary"><i class="fas fa-copy"></i> Duplicar Despesa</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-8">
                            <label for="duplicar_pix">Nº Chave Pix</label>
                            <input type="text" class="form-control" id="duplicar_pix" name="pix">
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="duplicar_cliente_id">Cliente</label>
                        <select class="form-control" id="duplicar_cliente_id" name="cliente_id" required>
                            <option value="">Selecione...</option>
                            <?php foreach ($clientes as $cliente) : ?>
                                <option value="<?= htmlspecialchars($cliente['id']) ?>"><?= htmlspecialchars($cliente['nome']) ?></option>
                            <?php endforeach; ?>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="duplicar_remarks" name="remarks" rows="2"></textarea>
                    </div>
                    <hr>

                    <div class="form-group">
                        <label for="pdfFileInput_duplicar">Anexar Comprovante(s) (PDF)</label>
                        <input type="file" id="pdfFileInput_duplicar" class="form-control-file auto-upload" accept="application/pdf" multiple data-status-div="uploadStatus_duplicar" data-anexos-input="anexos_json_duplicar">
                        <div id="uploadStatus_duplicar" class="mt-2 small upload-status"></div>
                    </div>
                    <input type="hidden" name="anexos_json" id="anexos_json_duplicar">
    ...
    <script>
        $(document).ready(function() {
            ...
            // Ação para o botão DUPLICAR (VERSÃO CORRIGIDA)
            $('.duplicar-btn').on('click', function() {
                ...
                const cliente_id = $(this).data('cliente_id'); // Corrigido para pegar o ID do cliente
                const remarks = $(this).data('observacoes');

                // Preenche os campos do modal de duplicar
                $('#duplicar_amount').val(valor);
                $('#duplicar_category_id').val(categoria);
                $('#duplicar_pix_type').val(pix_type);
                $('#duplicar_pix').val(pix);
                $('#duplicar_cliente_id').val(cliente_id); // Corrigido para usar o ID
                $('#duplicar_remarks').val(remarks);

                // Limpa a área de status de upload anterior
                $('#uploadStatus_duplicar').html('');
                $('#anexos_json_duplicar').val('');
            });
            ...
            // limpeza do modal de duplicar
            $('#duplicarModal').on('show.bs.modal', function(event) {
                const anexosInputId = 'anexos_novos_json_duplicar';
                anexosAtuais[anexosInputId] = [];
                $('#' + anexosInputId).val('');
                $(this).find('.upload-status').html('');
                $(this).find('.auto-upload').val('');
                $('#listaAnexos_duplicar').html('');
            });
        });
    </script>
</body>
