...
import {
  TicketStatus,
  type ImprovementListFilters,
  type ImprovementListSort,
  type ImprovementSortDirection,
} from '@helpdesk/contracts';

export interface ParsedImprovementListQuery {
  page: number;
  limit: number;
...
const ALLOWED_SORTS = new Set<ImprovementListSort>([
  'id',
  'client',
  'openedAt',
  'level',
...
function statuses(value: unknown): TicketStatus[] {
  const values = integerList(value, 'status', DEFAULT_STATUSES);
...
function sortValue(value: unknown): ImprovementListSort {
  const text = textValue(value) as ImprovementListSort | undefined;
...
function directionValue(value: unknown): ImprovementSortDirection {
...
export function parseImprovementListQuery(
  query: Record<string, unknown>,
): ParsedImprovementListQuery {
  ...
  const improvementId = positiveInteger(query.id, 'id');
