...
export async function GET(
  _request: Request,
  context: {
    params: Promise<{ expenseId: string; attachmentKey: string }>;
  },
): Promise<Response> {
  ...
  const upstream = await fetch(
    `${internalApiUrl()}/logistics/expenses/admin/approvals/${expenseId}/attachments/${encodeURIComponent(attachmentKey)}/content`,
    {
      cache: 'no-store',
      headers: { Accept: 'application/pdf', Cookie: cookieHeader },
    },
  );
  ...
  if (!upstream.ok) {
    return new Response(
      upstream.status === 403
        ? 'Sem permissão para abrir o comprovante.'
