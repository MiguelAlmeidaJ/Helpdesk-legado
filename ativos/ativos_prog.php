...
//ajuste manual para limitar o id empresa ver os ativos de outras

// Verifica o ID do usuário da sessão e ajusta a consulta
if (isset($_SESSION['allterusN3Id']) && $_SESSION['allterusN3Id'] == 58) {
  // Ajusta a consulta para o ID específico
  $baseQuery .= " AND LOWER(empresa) LIKE LOWER(:empresa_pharmes)";
  $bindings[':empresa_pharmes'] = '%pharmes%';
}

// Verifica o ID do usuário da sessão e ajusta a consulta
if (isset($_SESSION['allterusN3Id']) && $_SESSION['allterusN3Id'] == 164) {
  // Ajusta a consulta para o ID específico
  $baseQuery .= " AND LOWER(empresa) LIKE LOWER(:empresa_ativa)";
  $bindings[':empresa_ativa'] = '%ativa%';
}

// Verifica o ID do usuário da sessão e ajusta a consulta
if (isset($_SESSION['allterusN3Id']) && $_SESSION['allterusN3Id'] == 105) {
  // Ajusta a consulta para o ID específico
  $baseQuery .= " AND LOWER(empresa) LIKE LOWER(:empresa_super_m)";
  $bindings[':empresa_super_m'] = '%super_m%';
}

// Verifica o ID do usuário da sessão e ajusta a consulta
if (isset($_SESSION['allterusN3Id']) && $_SESSION['allterusN3Id'] == 40) {
  // Ajusta a consulta para o ID específico
  $baseQuery .= " AND LOWER(empresa) LIKE LOWER(:empresa_cardao)";
  $bindings[':empresa_cardao'] = '%cardao%';
}
...
// echo "<script>console.log('Quantidade de ativos encontrados para a página atual: $qtPaginated');</script>";
// echo "<script>console.log('IDs encontrados na página atual: " . implode(', ', array_column($ativos, 'id')) . "');</script>";
// echo "<script>console.log('Primeiro ID exibido: $firstIdOnPage');</script>";
// echo "<script>console.log('último ID exibido: $lastIdOnPage');</script>";
// echo "<script>console.log('Ordem atual: $ord');</script>";
echo "<script>console.log('Limite atual: $limit');</script>";
...
function buildPaginationUrl($page, $lastId = null)
{
  $params = [
    'page' => $page,
    'id' => $_POST['id'] ?? ($_GET['id'] ?? ''),
    'hora_coleta' => $_POST['hora_coleta'] ?? ($_GET['hora_coleta'] ?? ''),
    ...
    'sistema_operacional' => $_POST['sistema_operacional'] ?? ($_GET['sistema_operacional'] ?? ''),
    'armazenamento_disco_total' => $_POST['armazenamento_disco_total'] ?? ($_GET['armazenamento_disco_total'] ?? ''),
    'armazenamento_disco_uso' => $_POST['armazenamento_disco_uso'] ?? ($_GET['armazenamento_disco_uso'] ?? ''),
    'armazenamento_disco_livre' => $_POST['armazenamento_disco_livre'] ?? ($_GET['armazenamento_disco_livre'] ?? ''),
    'armazenamento_porcentagem' => $_POST['armazenamento_porcentagem'] ?? ($_GET['armazenamento_porcentagem'] ?? ''),
...
<head>
  ...
  <style>
    ...
    .coluna-folder {
  width: 50px; /* Define a largura da coluna */
...
.coluna-folder .dropdown {
  position: relative; /* Mantém o dropdown dentro do fluxo da célula */
}

.coluna-folder .dropdown-menu {
  position: absolute; /* Permite que o dropdown expanda além da célula */
  left: 0; /* Alinha o dropdown ao lado esquerdo */
  width: auto; /* Permite que o dropdown tenha uma largura automática */
  min-width: 150px; /* Define uma largura mínima para o dropdown */
  z-index: 1050; /* Garante que o dropdown esteja acima de outros elementos */
}


    .dropdown-folder {
  outline: none !important; /* Remove o contorno no foco */
  box-shadow: none !important; /* Remove o efeito de destaque */
  background-color: transparent !important; /* Garante que o fundo não mude */
    }

.dropdown-folder:hover, .dropdown-folder:focus, .dropdown-folder:active {
  outline: none !important; /* Remove o contorno no hover, foco e clique */
  box-shadow: none !important; /* Remove qualquer sombra no hover */
  background-color: transparent !important; /* Garante consistência no fundo */
            ...
            <form action="#" method="POST">
              <div class="form-row align-items-center">
                <div class="col-auto col-form-label-sm">
                  ...
                  <input type="text" id="filtro_empresa" name="empresa" class="form-control form-control-sm my-1" value="<?php echo htmlspecialchars(str_replace('%', '', strtolower($empresa))); ?>" tabindex="1">
                </div>
                <div class="col-auto col-form-label-sm">
            ...
            <tbody>
              <?php if (!empty($ativos)): ?>
                  ...
                  <tr>
                    ...
                    <td class="align-middle p-1 align-middle">
                      <form action="ativos_edit.php" method="GET" class="d-inline">
                        <input type="hidden" name="id" value="<?php echo $ativo['id']; ?>">
                ...
                <div style="text-align: center;">
                  <?php if ($page > 1): ?>
                    <a href="<?php echo buildPaginationUrl(1); ?>" style="margin: 0 5px;"><<</a>
